(function () {
    ...
    try {
      ...
      el.innerHTML = `
        <div class="stats-grid">
          ...
          <div class="card stat-card"><div class="value">${tb.balance ?? '-'}</div><div class="label">Баланс Twiboost ${tb.currency || ''}</div></div>
        </div>`;
    } catch (e) {
  ...
  async function loadAdminServices() {
    ...
    el.innerHTML = `<div class="table-wrap"><table>
      <thead><tr><th>ID</th><th>Название</th><th>Цена</th><th>Наценка %</th><th>Активна</th><th></th></tr></thead>
      <tbody>${list.map((s) => `
        <tr>
          <td>${s.id}</td>
          <td>${escape(s.name)}</td>
  ...
  async function loadAdminOrders() {
    ...
    el.innerHTML = rows.length ? `<div class="table-wrap"><table>
      <thead><tr><th>#</th><th>Email</th><th>Услуга</th><th>Сумма</th><th>Статус</th></tr></thead>
      <tbody>${rows.map((o) => `<tr><td>${o.id}</td><td>${escape(o.email)}</td><td>${escape(o.service_name)}</td><td>${o.cost_rub}</td><td>${escape(o.status)}</td></tr>`).join('')}</tbody></table></div>`
  ...
  async function loadUsers() {
    ...
    el.innerHTML = `<div class="table-wrap"><table>
      ...
      <tbody>${rows.map((u) => `<tr><td>${u.id}</td><td>${escape(u.email)}</td><td>${u.role}</td><td>${u.balance_rub}</td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  async function loadSettings() {
    ...
    Object.keys(s).forEach((key) => {
      ...
      if (input && !s[key].is_sensitive) input.value = s[key].value;
    });
